...
class MedicionesController extends Controller {
    ...
    public function index() {
        ...
        $consula = mediciones::withTrashed()->join('sensores','mediciones.id_sensor','=','sensores.id_sensor')->
            SELECT('mediciones.id_mediciones','mediciones.humedad_m','mediciones.temperatura_m','mediciones.medicion',
                'mediciones.fecha_m','mediciones.hora_m','mediciones.deleted_at',
                    'sensores.tiposensor as sensor')->get();
        return view('medicion.index')->with('consulta',$consula);
    }

    public function create() {
        $consula = mediciones::orderBy('id_mediciones','DESC')->take(1)->get();
            $cuantos = count($consula);
                if ($cuantos === 0) {
                    $idsiguiente = 1;
                } else {
                    $idsiguiente = $consula[0]->id_mediciones + 1;
                }
